<template>
  <view class="page-panel" :style="[THEME_CSS_VAR]">
    <u-sticky bgColor="#fff" offsetTop="0" customNavHeight="0">
      <u-tabs :current="0" :list="tabList" :scrollable="false" :lineWidth="60" :lineHeight="2" @click="tabChange"
      :activeStyle="{fontWeight: '400',fontSize: '26rpx',color: '#2761FF'}" 
      :inactiveStyle="{fontWeight: '400',fontSize: '26rpx',color: '#666666 '}"/>
      <dropdown-filter :pageType="searchForm.selTabId" @confirm="dropdownEvent" />
    </u-sticky>

    <template v-if="searchForm.selTabId == 'selfTask'">
      <self-sheet v-for="(item, index) in dataList" :key="index" :data="item" @click="recordPageNum(index)" />
    </template>
    <template v-if="searchForm.selTabId == 'allTask'">
      <all-sheet v-for="(item, index) in dataList" :key="index" :data="item" @click="recordPageNum(index)" />
    </template>
...
export default {
  ...
  data() {
    return {
      tabList: [
        { name: '我的任务', id: 'selfTask' },
        // { name: '总任务列表', id: 'allTask' }
      ],
      searchForm: {
        selTabId: 'selfTask',
        creator: '', // 创建人
  ...
  methods: {
    ...
    async getData(pageIndex = 1, options = {}) {
      ...
      if (pageIndex == 1) {
        ...
        // 已经请求完全部数据（最后一页），如果不是更新某一页数据，则终止操作；如果是想更新某一页的数据，则可以操作
