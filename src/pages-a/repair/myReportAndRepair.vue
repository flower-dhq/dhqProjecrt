...
<script>
	...
	export default {
  	  ...
  	  return {
  	    title: '物业报事报修',
  	    path: '/pages-a/repair/myReportAndRepair?categories='+this.thatOptions.categories+'&taskType='+this.thatOptions.taskType+'&title='+this.thatOptions.title
  	  }
  	},
		...
		methods: {
			...
			loadListData(params) {
				...
				args.data.orderType = indexType ? 2 : 1;
				this.isNoDataBack = false
				this.isEndData= false;
				uni.showLoading({title:'加载中...'})
				const p = feedBackOrderLists(args);
				...
				return p.then(res => {
					...
					uni.hideLoading()
					if (res.status === '1') {
						const list = res.hasOwnProperty('data') && Array.isArray(res.data) && res.data || [];
						...
						list.forEach(item => {
							...
							const rc = {
								serviceNo: orderNum,
								status: 1,
								statusText: state == '已暂停' ? '处理中' : state ,
								desc: remarks,
								time: createDate,
								orderId,
								managerTel,
						...
						this[key].isNoData = this[key].list.length < totalCount ? false : true
			...
			async buttonEvent(params) {
				...
				if (type === 'progress') {
					...
					uni.navigateTo({
						url:'/pages-a/repair/processingProgress?arg='+encodeURIComponent(JSON.stringify(arg)),
					})
				} else if (type === 'tel') {
