    ...
    <view :class="[THEME_NAME]" :style="[THEME_CSS_VAR]">
        ...
        <view class="box">
            ...
            <view class="head_detail">
                ...
                <view class="">
                    <text class="act-detail">活动简介</text>
                    <text>{{activityInfo.description}}</text>
                </view>
                <a v-show="activityInfo.summaryUrl" class="detail_btn" @click="openPage">查看详情 ></a>
                <a v-show="activityInfo.summaryType == 3" class="detail_btn" @click="openQusetionPage">查看详情 ></a>
            </view>
        ...
        <view class="activity-sign" v-if="!regInformationStart">
            <uni-row v-for="(item,index) in signUpInfos" :key="index" class="demo-uni-row">
                <uni-col span="5" class="hdCellleft">
                    <view class="sign-title"> {{item.title}}<i></i></view>
                </uni-col>
                <uni-col span="19">
                    <u-input v-model="item.value" :readonly="readOnly" :type="item.type" holdKeyboard :placeholder="item.placeholder" clearable border="none" fontSize="14"></u-input>
										<!-- <view class="clear-btn" @click="ckCrear(index)"></view> -->
								</uni-col>
            </uni-row>
            <uni-row >
                <uni-col span="5.5" class="hdCellleft">
                    <view class="sign-titles">参与人数 <i></i></view>
                </uni-col>
                <uni-col span="18.5">
            ...
            <uni-row v-if="activityInfo.isFree==0">
                <uni-col span="5.5" class="hdCellleft">
                    <view class="sign-titles">合计费用 <i></i></view>
                </uni-col>
                <uni-col span="18.5">
                    <u-input v-model="totalMoney" disabled="true" disabledColor="#fff" :readonly="readOnly" type="number" border="none" fontSize="14"></u-input> 
                </uni-col>
            </uni-row>
            <view v-if="activityInfo.isFree==0">
                ...
                <view class="sign-zf">
                    <u-radio-group v-model="payType" placement="column">
                        ...
                        <view class="sign-zfcell" v-if=" uniPlatform === 'app'">
                            <view class="cell-left">
        ...
        <u-modal :show="showTip" title="提示" :width="260" @confirm="goback" :show-confirm-button="isAcitityAuto">
            <view class="slot-content">
                <text>此活动为业主专享活动，只有业主才能参加此活动哟</text>
            </view>
        </u-modal>

        <u-modal :show="showTipss" title="提示" :show-confirm-button="false">
            <view class="slot-content">
                <text>{{ resMsg }}</text>
            </view>
        </u-modal>
        <u-modal :show="isAutoLogin" title="提示" @confirm="autoLogin">
            <view class="slot-content">
                <text>请先授权哟</text>
            </view>
        </u-modal>
        ...
        <u-modal :show="payModal" title="支付提示" @confirm="payModal=false">
            <view class="slot-content">
        ...
        <u-popup :show="subTips" mode="center" round='5'>
            <view class="tips-box">
                <image src="@/static/activity/success_icon.png"></image>
                <text class="text">报名成功</text>
                <view class="btn" @click="subOk">确定</view>
            </view>
        </u-popup>
...
import { saveTbgOrder, getPayChannel, getPay2 ,findLocations} from '../../service/community'
...
export default {
    ...
    onLoad(e) {
        ...
        let prePage = prePages.route
         if( prePage != 'pages-d/activity/activityList' && prePage != 'pages/index/index' && !this.memberId){
    ...
    methods: {
                ...
                if (res.statusCode == 200) {
                    ...
                    if (that.activityInfo.isMember == 1 && (userType != '业主')) {
                        ...
                        if(routes.length >=2){
                            let prePages = routes[routes.length - 2]; //获取路由参数 
                            let prePage = prePages.route
                            if( prePage == 'pages-d/activity/activityList' || prePage == 'pages/index/index' ){
                              this.isAcitityAuto = true
                            }
                        }
                        
                    }
                    let routes1 = getCurrentPages(); // 获取当前打开过的页面路由数组
                    if(routes1.length >=2) {
                        let prePages1 = routes1[routes1.length - 2]; //获取路由参数 
                        let prePage1 = prePages1.route
                        if(prePage1 != 'pages-d/activity/activityList' && prePage1 != 'pages/index/index' &&prePage1 !='pages-c/my-activitiy/my-ctivityList' && (res.data.checkMsg !='' && res.data.checkMsg !=undefined )){
                            that.showTipss = true;
                            // that.resMsg = res.data.checkMsg;
                            that.resMsg ='此活动为业主专享指定项目活动，只有指定项目的业主才能参加此活动哟';
                        }
                    }
                    ...
                    if (signUpEndDate > newTime) {
                        that.isSignBtn = true
                        console.log("that.isSignBtn = true");
                    } else {
                        console.log("signUpEndDate", signUpEndDate);
        ...
        openQusetionPage(){
            let that = this;
            if(that.activityInfo.summaryId){
               uni.navigateTo({ url: `/pages-c/questionnaire/detail?id=${that.activityInfo.summaryId}` }); 
            }
        },
        cancelSign() {
            //    取消报名
            let that = this;
            that.$refs.customModal.show({
                            content: '您确认要取消报名吗？',
                        ...
                        cancelSignUpByAsuid(params).then(res => {
                            if (res.status == 200) {
                                ...
                                setTimeout(() => {
                                    uni.redirectTo({
                                      url: `/pages-d/activity/activityDetail?aid=${that.aid}`    
                                   });
                                }, 600)    
                                // uni.reLaunch({
                                //   url: `/pages-a/activity/activityDetail?aid=${that.aid}`
                                // })
                                // location.reload();  
                            } else {
                                that.$refs.uToast.show({
        ...
        //收费活动
        async submitSign() {
            console.log(656,this.isLoading)
            if(this.isLoading){
				return
			}
			this.isLoading = true
        ...
        goToPay(args = {}) {
            ...
            console.log(885,localStorage)
            let { O_USER_INFO} = that.$constant;
            let { STRG_openId} = util.storageAction({ key: O_USER_INFO, action: 'get' });
            that.openId = STRG_openId
            console.log(886,STRG_openId,that.openId)
           ...
           console.log(983,tradeType)
            var params = {
                data: {
                    ...
                    projectId,
                    type: payType == 2 ? 1 : 0,
                    payMethod:this.uniPlatform == "mp-weixin" ? "wx" : 'zfb',
                    tradeType:tradeType,
                    returnUrl: returnUrl,
                    assetsId: assetsId,
                    notifyUrl: notifyUrl,
