...
/**
 * @description: 对指标箭头隐藏及修改指标值的颜色
 * @param {*} uCls 上升的类名
 * @param {*} dCls 下降的类名
 * @param {*} dfCls 默认类名
 * @param {*} isClsNm 是否类名
 * @param {*} val 指标值
 * @return {isUp: boolean, isDown: boolean}
*/  
export const indexArrow = (params = {}) => {
  const { uCls = 'item4', dCls = 'item5', dfCls = 'item3', val } = params
  const isUp = parseFloat(val) > 0
  const isDown = parseFloat(val) < 0

  return {
      clsName: isUp ? uCls : isDown ? dCls : dfCls,
      isUp,
      isDown
  }
}