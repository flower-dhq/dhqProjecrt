...
import { getAppShareConfig } from '@/service/api-gateway'
import { staffLogin,staffproviderLogin } from '@/service/wechatguanjia';
const { URL_PROJECT_TYPE, O_USER_INFO} = constant;
 ...
 */
const getThemeData = () => {
  const { THEME_NAME, IMG_BASE_URL, IMG_BASE_FOLDER} = utils.getAppConfig();
...
const getProjectConfig = () => {
  ...
  const webProjectType = PROJECT_TYPE; // uni.getStorageSync(URL_PROJECT_TYPE); // web暂时固定死
  const webCompanyId = COMPLANY_ID; // web暂时固定死
...
const setOwerUserInfo = (plaform, data) => {
  ...
  const local = utils.storageAction({ key: O_USER_INFO, action: 'get'}) || {};
  ...
  const useinfo = {
    ...
    loginPage() {
      ...
      return {
        ...
        STRG_birthDay: birthDay && utils.formatDate({value: Number(birthDay), type: 's2d', format: 'YY-MM-DD'}) || '',
    ...
    // 公众号授权接口
    h5Weixin() {
      const { memberId, wxHeadImgUrl, memberName, openId, memberPhone, wxUserName} = data || {}
      // 对应接口暂无， 用已存在的
...
const mallLoginByMbid = () => {
  ...
  const wyToken = utils.storageAction({ key: TOKEN, action: 'get' });
...
// 企微授权
const wxWorkAuth = function() {
  console.log('企微');
  return new Promise((resolve, reject) => {
    const SUITE_ID = Vue.prototype.$appConfig.SUITE_ID || ''

    wx.qy.login({
      suiteId: SUITE_ID,
      success: function(res) {
        console.log('=========wx.qy.login', res)
        if (res.code) {
          const companyId = Vue.prototype.$appConfig.COMPLANY_ID
          ...
          const result = SUITE_ID ? staffproviderLogin(params) : staffLogin(params)
          result.then(res => {
            console.log(SUITE_ID ? '==========staffproviderLogin' : '=========staffLogin222', res)
            if (res.status == 200) {
              const { IS_AUTH, TOKEN, O_USER_INFO, APP_TYPE, APPID_ENTIFIER } = constant
              ...
              // 获取业主端配置
              getAppShareConfig().then(res=> {
                console.log('业主端配置====', res)
                if(res.status == 200 && res.data){
                  // console.log('业主端配置====', res)
                  utils.storageAction({ key: APP_TYPE, action: 'set', value: res.data.appType })
                  utils.storageAction({ key: APPID_ENTIFIER, action: 'set', value: res.data.appIdentifier })
                }
              })

              if (res.data.isEnableQw == 0) {
                // 未开启企微配置
                uni.reLaunch({
                  url: '/pages-a/permission?errorCode=login'
                })
                resolve()
                return
              }
            ...
            console.log(SUITE_ID ? '==========staffproviderLogin' : '=========staffLogin', err)
            refreshPage()
            resolve()
          })
        } else {
  ...
  return new Promise((resolve, reject) => {
    dd.getAuthCode({
      success: function (res) {
        ...
        const result = staffLogin(params)
        result.then(res => {
          console.log( '=========staffLogin111', res)
          if (res.status == 200) {
...
const toolbarPermissions = function(){
  return new Promise((resolve, reject) => {
  const pages=getCurrentPages();
  let pagesRoute = pages && pages.length>0?pages[pages.length-1].route:'';
   let menuList = [{
      router: 'pages-workorder/my-workorder/auth-order',
      name:'业户工单',
      code:'toolbar-yhgd'
    }, {
      router: 'pages-archives/redirect-owner/index',
      name:'业户档案',
      code:'toolbar-yhda'
    }, {
      router: 'pages-leasing/selectProperty',
      name:'业户账单',
      code:'toolbar-yhzd'
    }, {
     router: 'pages-leasing/selectPropertyForTools',
     name:'业户账单',
     code:'toolbar-yhzd'
   }, {
      router: 'pages-a/commonPage/index',
      name:'常用链接',
      code:'toolbar-cylj'
    }];
   let router = menuList.find(item => pagesRoute == item.router) ?? {}
   let { O_USER_INFO } = constant;
    ...
    getFunctionButtons2(params).then(res=>{
      console.log('========获取菜单列表', params, res.data)
      if(res.resultCode == 200 && res.data){
         let data = res.data?.[0]?.items??[];
         let list = data.find(item => item.name == '会话工具栏')?.items ?? []
         if (!list || list.length <= 0) {
            resolve(false)
            console.log('========请开通“会话工具栏”菜单')
            uni.reLaunch({
              url: '/pages-a/permission?errorCode=menu'
            })
            return
         }

         const flag = list.some(element => element.menuCode == router.code)
         if (!flag) {
            resolve(false)
            uni.reLaunch({
              url: '/pages-a/permission?errorCode=menu'
...
function refreshPage() {
  uni.hideLoading()
  uni.showModal({
    content: '页面加载失败，请退出后重进',
    showCancel: false,
    confirmText: '确定',
    success() {
      const routes = getCurrentPages()
      const page = routes[routes.length - 1]
      if (!page || !page.$page.fullPath) return
  ...
  if(JSON.stringify(invokeOpt) != '{}' && ('data' in invokeOpt)){
    ...
    for(let key in invokeOpt.data){
      if(Array.isArray(invokeOpt.data[key]) || (typeof invokeOpt.data[key])=='object'){
        params[key] = JSON.stringify(invokeOpt.data[key])
      }
      if(!invokeOpt.data[key] && invokeOpt.data[key] == 'null'){
        params[key] = ''
      }else{
        params[key] = invokeOpt.data[key]
      }
    }
    // 统一转换数组对象    签名-start
    let dataList = parseQueryPostArr(params)

    // 数组对象排序
    dataList = sortByKey(dataList,params)
