    ...
    <view :class="[THEME_NAME,'keyPage']" :style="[THEME_CSS_VAR]">
      ...
      <view class="form-cont" style="background:#fff;">
        <u-form :model="formData" ref="uForm" labelWidth="80">
          <u-form-item label="用途" required border-bottom @click="isShow = true">
            <u-input v-model="formData.popuse" disabled="true" placeholder="家人使用" type="select" border="false"/>
            <u-icon slot="right" name="arrow-right"></u-icon>
          </u-form-item>
          <u-form-item label="借用人" required border-bottom>
        ...
        <view class="record-content">
          ...
          <view v-for="(item, index) in records" :key="index" :class="['cell', index === records.length - 1 ? '' : 'vux-1px-b']">
            <view class="cell-icon">
              <image class="image" src="/static/housekeeper/new-key-icon.png" width="40" height="40">
            </view>
      ...
      >
				<view class="slot-content">
					<view class="modelTxt">是否返回上一页</view>
				</view>
      </u-modal>
      <uniPop ref="rzModal" />
    </view>
...
<script>
    ...
    export default {
      ...
      onLoad() {
       ...
       this.companyId = util.storageAction({ key: COMPLANY_ID, action: 'get' }) ? util.storageAction({ key: COMPLANY_ID, action: 'get' }) :companyId
      },
        ...
        toGetAllApprovalList() {
          ...
          getOwnerBuildingListComm(params).then(response => {
            if (response.status == '200') {
              ...
              console.log(this.relateAddress)
              this.initData();
