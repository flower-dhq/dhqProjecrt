<template>
  <view v-if="isShowBar"
    class="flex commonHead top-bar-cont"
    :style="{ height: titleHeight + 'px' }"
  >
    <view class="commonHead-wrap flex">
      <!-- 0:不需要返回键，1：要home键，都不传表示要返回键（因为用到该按钮的页面比较多，所以不传代表显示返回键） -->
      <view class="commonHead_left">
        <view
          v-if="myProperty.flag!=0 && myProperty.flag!=1"
          class="commonHead_left_back"
          @click="commonHead_left_back"
        ></view>
        <view
          v-else-if="myProperty.flag == 1"
          class="commonHead_left_home"
          @click="commonHead_left_home"
        ></view>
      </view>
...
export default {
  ...
  data() {
    return {
      ...
      titleHeight: "",
      statusBarHeight: "",
      isShowBar:true //小程序才显示头部到导航
    };
  },

  mounted() {
    var that = this;
    //非小程序，不渲染该组件内容（在组件控制，方便以后适用其他平台的修改）
...
.top-bar-cont{
  ...
  top: 0;
  left: 0;
  background-image: linear-gradient(-90deg, #7de08b, #00c5d0);
}
.flex {
  display: -webkit-box;
  display: -moz-box;
