  ...
  <view :class="['dropdown-btn', getCustomClass]" :style="[THEME_CSS_VAR, getCustomStyle]" catchtouchmove="moveHandle">
    ...
    <view class="dropdown-box" :style="[{ zIndex: navIndex }]">
      ...
      <view :class="['content-panel', { show: maskShow, hide: !maskAnimate }]" :style="[getContentStyle]">
        <dropdown-project ref="dropdownP" :isAll="isAll" show @confirm="confirmEvent" :showProject="showProject" @getBuldList="getBuldList"/>
      </view>
    </view>
  </view>
</template>
...
import DropdownProject from './dropdown-newProject.vue'
...
export default {
  ...
  methods: {
		changeProject(projectId){
			this.$refs.dropdownP.selectDefaultProjectId(projectId)
		},
    ...
    confirmEvent(item) {
      let projectId = item.project.projectId + ''
			...
			if(!this.showProject){
				...
				this.label = item.building.buildingLocation || '全部'
			}else{
				this.label = !buildingId ? item.project.projectName : item.building.buildingLocation
			}
			this.selectProjectId = projectId
      this.showMaskEvent(false)
      this.$emit('confirm', { buildingId, projectId: projectId.includes('-all') ? '' : projectId })
    }
  }
}
</script>
