<template>
  <view :class="['dropdown-btn', getCustomClass]" :style="[THEME_CSS_VAR, getCustomStyle]" catchtouchmove="moveHandle"  style="margin-top: 14rpx;">
    ...
    <view class="dropdown-box" :style="[{ zIndex: navIndex }]">
      <view class="btn-panel" :style="[getNavigatorStyle]" @tap="showMenuEvent">
        ...
        <u-icon name="arrow-down-fill" size="20rpx" color="#ccc" top="3rpx" space="4rpx" labelPos="left" label=" " />
      </view>
      <view :class="['content-panel', { show: maskShow, hide: !maskAnimate }]" :style="[getContentStyle]">
        <dropdown-project show @confirm="confirmEvent" />
      </view>
    </view>
  </view>
</template>
...
import DropdownProject from './dropdown-noall-project.vue'
...
export default {
  ...
  methods: {
    ...
    confirmEvent(item) {
      ...
      let areaId = item.area.areaId + ''
      this.label = projectId.includes('-all') ? item.area.areaName : item.project.projectName
