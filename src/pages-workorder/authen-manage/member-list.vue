    ...
    <!-- 楼栋的下拉菜单 -->
      ...
      <select-building :show="buildObj.isShow" :searchForm="searchForm" :options="options" @confirmSlBd="dropdownEvent"></select-building>
		...
		<view class="room-box" v-for="(item, index) in dataList" :key="index">
			<view class="top">
				<view class="title">{{item.ownerName}} | {{item.ownerCalls || ''}}</view>
				<view class="title2">{{item | getAddressText}}</view>
			</view>
			<view class="bottom" v-if="item.authBuilding == '1'">
				<view class="btn1" @click="fakeThat(item)">短信邀请</view>
				<view class="btn2" @click="openChat(item)">发起会话</view>	
			</view>
		</view>
    <u-loadmore v-if="isLastPage || pageForm.loadMore == 'loading'" :status="pageForm.loadMore" line />

    <u-modal :show="modalShow" content="给该房间的业主发送邀请认证短信" asyncClose showCancelButton @confirm="confirmEvent" @cancel="cancelEvent" />
...
import { getNoAuthOwnerInfoPage, smsMessage, inviteMemberBySMS } from '@/service/community'
import { getQYInfoByOwnerId, syncOwnerInfo, getCompanyAgentMaterialList } from '@/service/api-gateway'
...
export default {
  ...
  filters: {
    // 如果是一套房产，则显示 院区到房号的完整地址；如果是多套房产，则显示 等x套房产
    getAddressText(item) {
      const _count = parseInt(item.houseSum) || 0
      const _addressList = item.detailAddressList || ''

      if (_count <= 1) {
        return _addressList
      } else if (_count > 1 && _addressList) {
        return _addressList.split(',').slice(0, 1) + ` 等${_count}套房产`
      } else {
        return ''
      }
    }
  ...
  methods: {
		 ...
		 */
		async openChat(item) {
		  const ownerId = item.ownerId || '' // 反馈人业主id
		  if (!ownerId) {
    ...
    chooseExternal() {
      wx.qy.selectExternalContact({
		...
		fakeThat(item) {
      // 判断手机号是否满足11位
      const phoneNumber = item.ownerCalls || ''
      if (phoneNumber.length != 11) {
        uni.showToast({ title: '该联系电话不支持发送短信', icon: 'none' })
        return
      }
    ...
    // 短信接口
		async fakeThat(item) {
			// 获取企微管家系统配置的常用链接，房屋认证的配置信息。链接格式：/pages/index/index?param1=a&param2=b
      ...
      const ownerPhone = item.ownerCalls || ''
      if (ownerPhone.length != 11) {
        ...
        return
			...
			// 获取需要发送邀请的业主
			const inviteOwner = [{ ownerName: item.ownerName, ownerPhone }]
      ...
      query = (query || '') + '&addrId=' + (item.addrId || '') // addrId: 业主未认证该房产
    ...
    async getData(pageIndex = 1) {
      ...
      const res = await getNoAuthOwnerInfoPage(params).finally(() => {
    ...
    // 发送短信
    async confirmEvent() {
      const res = await smsMessage()
      
      const success = res.status == '200'
      uni.showToast({ title: success ? '发送成功' : '发送失败', icon:'none' })
      if (success) {
