    ...
    <view class="info-warp">
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        ...
        <view class="border-bottom font28">
          ...
          <text class="gray">{{processDetail.contractCode}}</text>
        </view>
        <view class="border-bottom font28">
      ...
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        ...
        <view style="padding: 0 30rpx;">
          <view class="hetong-item" v-for="(item,index) in contentList" :key="index">
            <view class="flex-sb" style="padding-bottom: 24rpx;">
              <text class="font30 grey">{{item.resourceName}}</text>
              <text class="orange font30" :class="[item.itemTypeText=='提前退租'?'red':'',item.itemTypeText=='续租'?'green':'']">{{item.itemTypeText}}</text>
            </view>
            <view class="font-28 mb-10">
          ...
          <!-- <u-gap v-if="index+1 < contentList.length" height="1" bg-color="#EBEBEB"></u-gap> -->
          <!-- <view style="border-bottom: 1px solid EBEBEB"></view> -->
        </view>
      </view>
    </view>
...
import { 
  ...
  getResourceContractInfo,  //资源合同详情
...
export default {
  ...
  onLoad(option) {
    ...
    this.getResourceContractDetail()
    this.getFileList()
  },
  methods: {
    // 获取资源合同详情
    getResourceContractDetail() {
      let params = {
        data: {
          serialNumber: this.serialNumber
        }
      }
      getResourceContractInfo(params).then(res=>{
        console.log('资源====',res)
        this.processDetail = res?.data?.baseInfo || {};
        this.contentList = res?.data?.itemInfo || [];
