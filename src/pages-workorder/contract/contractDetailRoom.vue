  ...
  <view class="page-panel" :style="[THEME_CSS_VAR]">
    ...
    <view class="info-warp">
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        ...
        <view class="border-bottom font28" style="margin-top: 20rpx;">
        ...
        <view class="border-bottom font28">
          ...
          <text>{{processDetail.contractName}}</text>
        </view>
        <view class="border-bottom font28">
          <text class="laber-name">工位数：</text>
          <text>{{processDetail.stationNumber}}</text>
        </view>
        <view class="border-bottom font28">
          <text class="laber-name">备注：</text>
          <text>{{processDetail.remark}}</text>
        </view>
      </view>
    </view>
      ...
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        ...
        <view style="padding: 0 30rpx;">
          <view class="hetong-item" v-for="(item,index) in contentList" :key="index">
            <view class="flex-sb" style="padding-bottom: 30rpx;">
              <text class="font-weight font30">{{item.itemTypeText}}</text>
              <text class="orange font28">{{item.itemStatusText}}</text>
            </view>
            <view class="mb-10">
              房号：{{item.houseRoom}}
            </view>
            <view class="mb-10">
              租赁时间：{{item.startDate}} ~ {{item.endDate}}
            </view>
            <view class="font28">
              记录：{{item.terminationReasons}}
            </view>
          </view>
        </view>
      </view>
      ...
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        ...
        <view style="padding: 0 30rpx;">
          <view class="money-item" v-for="(item,index) in bailLists" :key="index">
            <view class="flex-sb mb-28">
              <text class="font-30 grey">{{item.fiName}} | ￥{{item.receivableAmount}}</text>
              <text class="font-30 font-weight grey">{{item.chargePeriod}}</text></text>
            </view>
            <view class="icon-line" style="margin-bottom: 24rpx;">
              <view class="iconItem">
                <image class="img-icon" :src="require(`@/static/${[THEME_NAME]}/hetong/icon-yi.png`)" />
                <text>￥{{item.receivedAmount}}</text>
              </view>
              <view class="iconItem">
                <image class="img-icon" :src="require(`@/static/${[THEME_NAME]}/hetong/icon-tui.png`)" />
                <text>￥{{item.refundAmount}}</text>
              </view>
            </view>
            <view class="icon-line">
              <view class="iconItem">
                <image class="img-icon" :src="require(`@/static/${[THEME_NAME]}/hetong/icon-chong.png`)" />
                <text>￥{{item.deductAmount}}</text>
              </view>
              <view class="iconItem">
                <image class="img-icon" :src="require(`@/static/${[THEME_NAME]}/hetong/icon-yu.png`)" />
                <text>￥{{item.arrearsAmount}}</text>
              </view>
            </view>
          </view>
        </view>
    ...
    <view class="money-warp" v-if="collaterList.length">
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        <view class="line-warp">
          <text>押金</text>
        </view>
        <view class="money-item" v-for="(item,index) in collaterList" :key="index">
          <view class="flex-sb mb-28">
            <text class="font-30">{{item.fiName}} | ￥{{item.receivableAmount}}</text>
            <text class="font-30 font-weight">{{item.chargePeriod}}</text>
          </view>
          <view class="icon-line" style="margin-bottom: 24rpx;">
            <view class="iconItem">
              <image class="img-icon" :src="require(`@/static/${[THEME_NAME]}/hetong/icon-yi.png`)" />
      ...
      <view style="background: #fff;border-radius: 12rpx; padding-bottom: 18rpx;">
        ...
        <view class="border-bottom-30 flex-sb" v-for="(item, index) in fileLists" :key="index">
          ...
          <view class="flex">
            ...
            <!-- <text class="font-30 blue" @click="previewFile(item)">查看</text> -->
  ...
  methods: {
    ...
    getContractDetail() {
      ...
      getHouseContractInfo(params).then(res => {
        ...
        this.collaterList = res?.data?.houseContractCollaterInfo || [];
