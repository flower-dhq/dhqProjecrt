      ...
      <view class="common-card" v-if="resultData">
        ...
        <view class="health-level-section">
          ...
          <view class="level-display">
            <text class="level-number">{{ healthLevel }}级</text>
		  ...
		  <view class="progress-container">
			<view class="progress-levels">
			  <view 
				...
				:class="{ 'active': (7 - level) === healthLevel }"
			  >
				<view class="level-box">
				  <text class="level-text">{{ 7 - level }}级</text>
				  <text class="level-name">{{ getLevelName(7 - level) }}</text>
				</view>
			  </view>
			</view>
        ...
        <view class="attitude-section">
          ...
          <view class="attitude-table">
            <view class="attitude-row">
              <text class="attitude-label">是否夸大自身问题</text>
              <text class="attitude-result">{{ getAttitudeResult('A21') }}</text>
            </view>
            <view class="attitude-row">
              <text class="attitude-label">是否隐瞒自身问题</text>
              <text class="attitude-result">{{ getAttitudeResult('B21') }}</text>
            </view>
            <view class="attitude-row">
              <text class="attitude-label">是否匆忙回答问题</text>
              <text class="attitude-result">{{ getAttitudeResult('E14') }}</text>
            </view>
          </view>
        </view>
      </view>
        ...
        <view class="completion-info">
          ...
          <view class="info-item">
            ...
            <view class="info-content">
              ...
              <text class="info-value">{{ formatTime(new Date(endTime)) }}</text>
            </view>
          </view>
        </view>
...
export default {
  ...
  computed: {
    ...
    duration() {
      ...
      if (!obj.startTime) return 0
      return Math.round(((obj.endTime?new Date(obj.endTime):new Date()) - new Date(obj.startTime)) / 1000 / 60)
    },
    healthLevel() {
  ...
  methods: {
    ...
    async getResult(){
      ...
      if(tmpResult.code == 20000){
        this.resultData = tmpResult.data
        questionInfo.startTime = tmpResult.data.startTime
        questionInfo.endTime = tmpResult.data.endTime
      }  
    },
    ...
    getIndicatorPosition() {
      // 计算当前等级在进度条中的位置（百分比）
      const position = ((6 - this.healthLevel) / 6) * 100 + 8
      return `${position}%`
    }
  }
}
