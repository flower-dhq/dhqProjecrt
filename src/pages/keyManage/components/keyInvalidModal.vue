<template>
    <div v-if="isVisible" class="full-screen-modal">
        <Card class="modal-content">
          <template #title>
            <div class="title">作废</div> 
          </template>
          <div class="column">
            
            <div class="row bottom-line" >
              <div class="left-td">作废原因</div>
              <div class="right-td" >
                <Input v-model="invalidReason" maxlength="500" show-word-limit type="textarea" placeholder="作废原因" style="width: 580px;" rows="5" />
              </div>
            </div>
            <div class="buttonRow">
          ...
          <!-- <table>
            ...
            <tr style="height:100px">
              ...
              <td class="right-td" >
                <Input v-model="formData.invalidReason" type="textarea" placeholder="审阅描述" style="width: 200px" />
              </td> 
            </tr>
          </table> -->
        </Card>
...
<script>
  ...
  import {invalidKeyInfo} from '@/service/officeSystem.js'
  export default {
    props: ['isVisible', 'id'],
    data(){
      return {
    ...
    methods:{
      submit(){
        ...
        invalidKeyInfo(parm).then(res=>{
          if(res.status == '200'){
            self.$Message.success("已作废")
            self.$emit('success',res.message)
          } else {
