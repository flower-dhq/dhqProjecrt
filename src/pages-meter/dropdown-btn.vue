<template>
  <view :class="['dropdown-btn', getCustomClass]" :style="[THEME_CSS_VAR, getCustomStyle]" catchtouchmove="moveHandle" style="margin: 14rpx 20rpx 0 30rpx;">
    ...
    <view class="dropdown-box" :style="[{ zIndex: navIndex }]">
      <view class="btn-panel" :style="[getNavigatorStyle]" @tap="showMenuEvent">
        ...
        <!-- <u-icon name="arrow-down" size="24rpx" color="#fff" top="3rpx" space="4rpx" labelPos="left" label=" " /> -->
      ...
      <view :class="['content-panel', { show: maskShow, hide: !maskAnimate }]" :style="[getContentStyle]">
        ...
        <dropdown-radio :show= true :value="labelCode" :data="outsideList" @confirm="confirmEvent" />
      </view>
    </view>
  </view>
</template>
...
export default {
  ...
  methods: {
    ...
    confirmEvent(item) {
      ...
      this.outsideName = item.label
      this.labelCode = item.name
