...
function drawGaugeDataPoints(categories, series, opts, config, context) {
  ...
  var totalAngle = 0;
  //判断仪表盘的样式：default百度样式，progress新样式 simple 自定义的样式
  if (gaugeOption.type == 'progress') {
    ...
    context.restore();
    //default百度样式
  } else if (gaugeOption.type == 'simple') {
     //画背景
     context.setLineWidth(gaugeOption.width);
     context.setLineCap('butt');
     for (let i = 0; i < categories.length; i++) {
    ...
    series = getGaugeArcbarDataPoints(series, gaugeOption, process);
    context.setLineWidth(gaugeOption.width);
    context.setStrokeStyle('#2761FF');
     context.setLineCap('butt');
    context.beginPath(); 
    context.arc(centerPosition.x, centerPosition.y, radius, gaugeOption.startAngle * Math.PI, series[0]._proportion_ * Math.PI, false);
    context.stroke();
     //画指针
     ...
     for (let i = 0; i < series.length; i++) {
       ...
       context.beginPath();
       context.setFillStyle(eachSeries.color);
       context.moveTo(gaugeOption.pointer.width, 1);
