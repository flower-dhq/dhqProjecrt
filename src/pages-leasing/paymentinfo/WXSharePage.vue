...
export default {
    ...
    onLoad(option){
        if (option.scene) {
            const params = commonMethod.getRouteParams(option, 'k', 'n')
            this.key = params.k;
            this.userName = params.userName ? decodeURIComponent(params.n) : "";
            this.wxMiniLogin();
        }else{
            this.key = option.key;
            this.payChannel = option.payChannel ? option.payChannel : "";
            this.receivedRemark = option.remark ? option.remark : "";
    ...
    methods:{
        ...
        //初始化数据
        initData(){
            const {PROJECT_ID, MEMBER_ID, USER_INFO } = this.$constant;
            this.memberId = util.storageAction({ key: MEMBER_ID, action: 'get' });
            const {PROJECT_CODE,PROJECT_TYPE, COMPLANY_ID,OA_APP_ID: configAppId } = this.$appConfig;
