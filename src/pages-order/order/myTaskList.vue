<template>
  <view class="order-list" :class="[THEME_NAME]" :style="[THEME_CSS_VAR]">
    <view class="list-search-bar">
      <search-bar :filters="[1, 2, 5, 6]" @onSearch="handleSetSearchData"></search-bar>
    </view>
    <mescroll-uni ref="mescrollRef" @init="mescrollInit" @down="downCallback" @up="upCallback" :down="downOption" :up="upOption" height="100%" top="100px">
      <list-item v-for="item in listData" :key="item.quesTaskId" :action="false" :itemData="item" :checkactive="checkactive" :isChecked="checkactive && checkVal(item)" @updateCheck="updateCheck"></list-item>  
    </mescroll-uni>
    <view class="btn-wrap" v-if="checkactive">
      <view class="btn" @click="saveCheck"> 确认 </view>
    </view>
  </view>
</template>
...
import { findMyTaskListByPage, getLastQuesTaskServerTypeList, getAllQuesTaskServerTypes } from '@/service/wuyecrm'
export default {
  name: 'order-list',
  components: { searchBar, listItem, MescrollUni },
  ...
  onLoad(option) {
    this.checkactive = option.checkactive == '1' ? true : false
    if(this.checkactive){
      this.setDataInfo({ tabType: '1' })
      const checked = uni.getStorageSync('relateTasks') || '[]'
      this.checkList = JSON.parse(checked)
    }
  ...
  methods: {
    ...
    selectArea(areaId) {
      ...
      getProjectAndQuesRange(params).then((res) => {
        if (res.resultCode == '200' && res.data && res.data.length > 0) {
          ...
          this.setDataInfo({ projectList: res.data })
          this.requestData()
        }
      })
    },
    ...
    requestData() {
      console.log(333, this.dataInfo.filterData)
      this.searchData.tabType = this.dataInfo.tabType
      ...
      this.searchData = { ...this.searchData, ...this.dataInfo.filterData }
    ...
    updateCheck(item){
      let Idx = this.checkList.findIndex(ele => ele.quesTaskId == item.quesTaskId)
      if(Idx > -1){
        this.checkList.splice(Idx, 1)
      }else{
        this.checkList.push({
          quesTaskId: item.quesTaskId,
    ...
    checkVal(item){
      return this.checkList.some(ele => ele.quesTaskId == item.quesTaskId)
    },
    saveCheck(){
      const eventChannel = this.getOpenerEventChannel()
      eventChannel.emit('dataEvent', this.checkList)
      uni.navigateBack()
    }
  }
}
