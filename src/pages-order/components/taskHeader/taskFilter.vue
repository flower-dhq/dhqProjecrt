...
export default {
  ...
  methods: {
    ...
    handleSelectItem(item) {
      ...
      if (this.menuId == 'time') {
        if (this.valueId == 'today') {
          ...
          this.endDate = this.$dayjs().endOf('week').format('YYYY-MM-DD')
    ...
    customChange(val) {
      if (this.customMode == 'date') {
        this.startDate = this.$dayjs(val.startDate).format('YYYY-MM-DD')
        this.endDate = this.$dayjs(val.endDate).format('YYYY-MM-DD')
      } else if (this.customMode == 'datetime') {
