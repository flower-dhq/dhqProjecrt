...
<template>
  <view>
    ...
    <view class="pay-box">
      <view class="pay-type">
      <view>
        <image v-if="paySuccess == '支付成功'" class="pay_success" :src="require(`@/static/theme-default/payment/Success.png`)"></image>
        <image v-if="paySuccess == '支付失败'"  class="pay_success" :src="require(`@/static/theme-default/payment/error.png`)"></image>
        <image v-if="paySuccess == '支付中'"  class="pay_success" :src="require(`@/static/theme-default/payment/loading.gif`)"></image>
        </view> 
      <text class="pay-class">{{paySuccess}}</text> 
      <!-- <button class="pay-btn" type="primary" open-type="launchApp" app-parameter="wechat"  binderror="launchAppError">我已支付</button> -->
      <button class="pay-btn" type="primary" @click="navBackMini">返回上一个小程序</button>
      </view> 
        <!-- <view class="pay-btn-box">require(`@/static/theme-default/payment/error.png`)
  ...
  data(){
    return{
      ...
      globalData:{
        ...
        unionId: null, //小程序unionId
        session_key: null, //session失效key
        userId: null, //业主用户信息
        basePath: 'https://property.gzbbn.com.cn',
        webUrl: 'https://property.gzbbn.com.cn',
        prefix: '/miniPrograms', // 地址前缀。没填的话默认为空。需要加 / 符号
        appid: 'wx13cd7bafd7144de2',
        code: "MDZlYTY4N2FhMzBiODg4MTNjY2FlNDkyYTQzZTljZjI=", // AppSecret(小程序密钥)	，然后使用base64加密，然后还得去运营中心里配置开发者密码
        projectType: 218,
        companyId: '482',
        wxId: 218
      },
  ...
  methods:{
    ...
    onloadFun(options){
      ...
      if(options.wxPaymentVersion && options.wxPaymentVersion==2){
        ...
        this.allParamsData = options.allParamsData && JSON.parse(options.allParamsData)
        console.log('参数NEW',this.allParamsData)
        that.orderInfo = options.orderInfo
        wx.showLoading();
        that.appletLogin(1); 
      }
    },
      ...
      wx.login({
        success (res) {
          if (res.code) {
            //发起网络请求 
            wx.request({
              url: 'https://property.gzbbn.com.cn/community/wechatInfo/appletLogin',//广州城投
              data: {
                code: res.code,
                appletId:'218',
              },
              ...
              success: function (res) {
                ...
                if (result.status == 200) { 
                  ...
                  if(self.globalData.webUrl){
                    // self.setData({
                    //   webUrl: that.globalData.webUrl+"indexNew?source=applet&unionId="+result.data.unionId
                    // }); 
                    // if(type==1){
                      self.onloadPay();
                    // }else{ 
    ...
    getPayUrl(){
      ...
      console.log('用户信息',self.globalData)
      wx.request({
        url: self.globalData.basePath + '/community/payCommon/getPay?_t='+new Date().valueOf(),
        data:params,
        header: {
          "Content-Type": "application/json",
        ...
        success: function (res) {
          ...
          if(result.status == 200){
            ...
            uni.showToast({ title: '支付失败！'+result.message, duration: 2000, icon: 'none' })
            wx.hideLoading();
          }
        },
        error: function () {
    ...
    toPay(pay){ 
      ...
      // 发起微信支付
      wx.requestPayment({
        timeStamp: !!payInfo.timeStamp ? payInfo.timeStamp : payInfo.timestamp,
        nonceStr: !!payInfo.nonceStr ? payInfo.nonceStr : payInfo.noncestr,
        package: payInfo.package,
        signType: payInfo.signType,
        paySign:  !!payInfo.paySign ? payInfo.paySign : payInfo.sign,
        success (res) {  
          uni.showToast({ title: '支付成功', duration: 2000, icon: 'none' })
          that.paySuccess = '支付成功'
    ...
    onloadPay(){ 
      let that = this;
      let item = that.allParamsData;
      wx.request({
        url: that.globalData.basePath +'/community/payCommon/v2/getPay',
        data: {
          ...
          returnUrl:decodeURIComponent(item.returnUrl),//支付完成跳转地址
          notifyUrl:decodeURIComponent(item.notifyUrl),//上送支付中心回调接口
          totalAmount:parseInt(item.totalAmount),//必填 金额（单位：分）
          body:decodeURIComponent(item.body),//订单描述
          attach:item.attach,//自定义参数JSON字符串
          thirdJson:item.thirdJson,//三方参数JSON字符串
          userId:item.userId,//用户ID
          userName:item.userName,//用户姓名
          openId: that.globalData.openId ,//用户支付ID，例:微信openId，支付宝用户唯一ID(productId)
        },
        header: {
          "Content-Type": "application/json"
        },
        ...
        success (res) {   
          if(res.data.status==200){
            ...
            if(JSONStr){ 
              ...
              uni.showToast({ title: '未获取到支付参数，请联系管理员！', duration: 2000, icon: 'none' })
