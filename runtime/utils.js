    ...
    config.module
      ...
      .options({
        ...
        name(pwd) {
          ...
          const suffix = pwd.split('/src/')[1]
          if (!suffix) return pwd

          return `${IMG_BASE_FOLDER}/` + suffix
        },
      });
  }
};
  ...
  apply(compiler) {
    ...
    if (modifyImgBaseUrl) {
      compiler.hooks.emit.tap({ name: 'transferImages' }, (compilation) => {
        const tabbarImg = ['tabbar_home.png', 'tabbar_mine.png','tabbar_code.png', 'tabbar_service.png', 'tabbar_mall.png','tabbar_industry.png'];
